...
public class PgErrorMessageParser {
    ...
    private final static Pattern STREAMID_VERSION_EXISTS_PTRN =
            Pattern.compile("Key \\(stream_id, version\\)=\\((.+), (\\d+)\\) already exists\\.");

    public static String parseVersionConflictError(String errorDetail) {
        ...
        if (m.find()) {
            ...
            return ErrorMessages.eventVersionConflict(streamId, Integer.parseInt(version));
        } else {
            return errorDetail;
        }
    }
